...
export default function () {
  ...
  group("CRUD cycle", () => {
    let createRes;
    group("POST create", () => {
      createRes = http.post(`${BASE_URL}/${ENDPOINT}`, CREATE_PAYLOAD, {
        headers: HEADERS,
    ...
    group("GET find_one", () => {
      const res = http.get(`${BASE_URL}/${ENDPOINT}/${id}`, {
        tags: { name: `GET /${ENDPOINT}/:id` },
      });
      const ok = check(res, {
    ...
    group("UPDATE", () => {
      ...
      const updateOpts = {
        headers: HEADERS,
        tags: { name: `${UPDATE_METHOD} /${ENDPOINT}/:id` },
      };
    ...
    group("DELETE", () => {
      const res = http.del(`${BASE_URL}/${ENDPOINT}/${id}`, null, {
        tags: { name: `DELETE /${ENDPOINT}/:id` },
      });
...
export function setup() {
  ...
  if (res.status !== 200) {
    throw new Error(
      `El servidor en ${BASE_URL} no responde. Status: ${res.status}. Abortando benchmark.`,
    );
  }
