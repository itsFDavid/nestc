def write_router(f, data):
    ...
    # 2. Cabeceras Globales (CORS y JSON)
    ...
    # 5. CAPA DE SEGURIDAD 1 (Anti Path Traversal)
    ...
    # 6. CAPA DE SEGURIDAD 2 (Límite de Payload - Max 1MB)
    ...
    # Responder al cliente (Usando las cabeceras CORS)
    f.write("  if (json_res == NULL) { json_res = strdup(\"{}\"); status_code = 500; }\n")
    f.write('  mg_http_reply(c, status_code, headers, "%s\\n", json_res);\n\n')
