def write_router(f, data):
    ...
    # 1. Declarar punteros e inicializar a NULL (Prevención de errores de scope)
    f.write("  char *method = NULL; char *uri = NULL; char *req_body = NULL;\n")
    ...
    # 2. Alojar Método y URI
    f.write("  method = malloc(hm->method.len + 1);\n")
    f.write("  if (method) snprintf(method, hm->method.len + 1, \"%.*s\", (int)hm->method.len, hm->method.buf);\n")
    f.write("  uri = malloc(hm->uri.len + 1);\n")
    f.write("  if (uri) snprintf(uri, hm->uri.len + 1, \"%.*s\", (int)hm->uri.len, hm->uri.buf);\n\n")
    
    f.write("  if (!method || !uri) goto cleanup;\n\n")

    # 3. Manejo de CORS Preflight
    f.write("  if (strcmp(method, \"OPTIONS\") == 0) {\n")
    f.write("    mg_http_reply(c, 204, headers, \"\");\n")
    f.write("    goto cleanup;\n  }\n\n")

    # 4. CAPA DE SEGURIDAD (Check de ".." antes de normalizar)
    ...
    # 5. Payload Limit y Extracción de Body
    ...
    f.write("  req_body = malloc(hm->body.len + 1);\n")
    f.write("  if (req_body) snprintf(req_body, hm->body.len + 1, \"%.*s\", (int)hm->body.len, hm->body.buf);\n")
    f.write("  else req_body = strdup(\"\");\n\n")

    f.write("  NestResponse nest_res = {0, NULL};\n")
    f.write("  int handled = 0;\n\n")

    # --- Lógica de Enrutamiento (Bucle de controllers) ---
    first = True
    for c_info in data["controllers"]:
        ...
        if http_method in ["POST", "PUT", "PATCH"]:
            call_args.append("&dto_obj" if dto_name else "req_body")
        ...
        if dto_name:
            ...
            f.write(f'{indent}  nest_res = NESTC_BAD_REQUEST(dto_err);\n')
        ...
        f.write(f'{indent}handled = 1;\n')
        if has_param: f.write('      free(raw_id);\n    }\n')
        f.write("  }\n")

    # 6. Finalización y Logger
    f.write("\n  if (!handled) nest_res = NESTC_NOT_FOUND(\"{\\\"error\\\": \\\"Route not found\\\"}\");\n")
    f.write("  if (!nest_res.body) nest_res = NESTC_ERROR(\"{\\\"error\\\": \\\"Internal Server Error\\\"}\");\n\n")
    ...
    f.write('  char *st_color = (nest_res.status >= 400) ? (nest_res.status >= 500 ? "\\x1b[31m" : "\\x1b[33m") : "\\x1b[32m";\n')
    f.write('  printf(\"\\x1b[35m[Nest-C]\\x1b[0m - %s  \\x1b[33m%s\\x1b[0m \\x1b[36m%s\\x1b[0m \\x1b[90m-->\\x1b[0m %s%d\\x1b[0m\\n\", time_buffer, method, uri, st_color, nest_res.status);\n\n')

    f.write("cleanup:\n")
    f.write("  if (nest_res.body) free(nest_res.body);\n")
    f.write("  if (req_body) free(req_body);\n")
    f.write("  if (uri) free(uri);\n")
    f.write("  if (method) free(method);\n")
    f.write("}\n")
